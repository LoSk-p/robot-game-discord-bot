    ...
    try:
        ...
        for device in recepient_addresses:
            try:
                ...
                print(f"Faild to encrypt key for: {device} with error: {e}. Check your RWS devices, you may have SR25519 address in devices.")
        data_final = {"encrypted_keys": encrypted_keys, "data": encrypted_data}
        return data_final
    except Exception as e:
        print(f"Exception in encrypt for devices: {e}")
...
def try_to_encrypt(receiver_address: str) -> bool:
    ...
    try:
        ...
        print(f"Can't encrypt test message with error: {e}")
        return False
